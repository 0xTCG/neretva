...
class Encoder(nn.Module):
    ...
    def __init__(self, num_mutations, num_alleles, num_sparse_entries, hidden, dropout):
        ...
        self.fcmu_base = nn.Linear(hidden // 2, num_sparse_entries * len(BASES))
        self.fclv_base = nn.Linear(hidden // 2, num_sparse_entries * len(BASES))
    ...
    def forward(self, inputs):
        ...
        logtheta_base_loc = logtheta_base_loc_flat.view(batch_size, self.num_sparse_entries, len(BASES))
        logtheta_base_logvar = logtheta_base_logvar_flat.view(batch_size, self.num_sparse_entries, len(BASES))
...
class VAE(nn.Module):
    ...
    def compute_allele_sparsity_loss(self, allele_props, gene_weights=None):
        ...
        for gene_name in self.db.genes:
            # Find alleles for this gene
            gene_allele_indices = [
                i for i, allele in enumerate(self.sample.valid_alleles) 
                if allele.gene.name == gene_name
            ]
            
            if len(gene_allele_indices) == 0:
                continue
            
            # Get probabilities for this gene's alleles
            gene_probs = allele_props[gene_allele_indices]
            
            # Normalize within gene (in case of normalization issues)
            gene_probs_norm = gene_probs / (gene_probs.sum() + eps)
            
            # Compute entropy: H = -sum(p * log(p))
            # We want LOW entropy (concentrated on few alleles)
            entropy = -(gene_probs_norm * torch.log(gene_probs_norm + eps)).sum()
            
            # Get gene-specific weight
            weight = gene_weights.get(gene_name, 5.0)  # Default to 5.0 if not specified
            
            gene_entropies.append(entropy)
            gene_weights_used.append(weight)
        
        
        gene_entropies = torch.stack(gene_entropies)
        gene_weights_tensor = torch.tensor(gene_weights_used, device=gene_entropies.device)
        
        weighted_entropy = (gene_entropies * gene_weights_tensor).sum()
    ...
    def loss_function(self, mutation_probs, mut_counts, 
        ...
        # strength_mask = self.get_strength_mask()
        strength_mask = torch.ones_like(self.get_strength_mask())
        ...
        loss = -log_likelihood + kld_weight * allele_kld + base_kld_weight * base_kld + functional_kld_weight*functional_kl + 100*base_entropy + 1*allele_sparsity_loss
...
def run_vae_single_seed(total_mut_counts, valid_alleles, mut_counts, num_sparse_entries, 
    ...
    for step in range(num_iterations):
        ...
        # beta_nonzero = (beta[0] != 0).sum().item()
        # print(f"Total non-zero entries in beta: {beta_nonzero}")
        ...
        loss, recon_loss, allele_kl, base_kl, functional_kl = model.loss_function(
            ...
            # functional_kld_weight = 6,
            functional_kld_weight = functional_kld_weight,
    ...
    with torch.no_grad():
        ...
        strength_mask = model.get_strength_mask()
        beta_squeezed = beta.squeeze(0)
        beta_adjusted = beta_squeezed * strength_mask
        row_sums = beta_adjusted.sum(dim=1, keepdim=True)
        beta_adjusted = beta_adjusted / (row_sums + 1e-10)
        
        # NOW print debug info
        print(f"\n=== DEBUG: Beta Adjusted ===")
        print(f"beta_squeezed shape: {beta_squeezed.shape}")
        print(f"beta_squeezed[:2, :5]: {beta_squeezed[:2, :5]}")
        print(f"beta_adjusted[:2, :5]: {beta_adjusted[:2, :5]}")
        print(f"beta_adjusted sum: {beta_adjusted.sum().item():.6f}")
        print(f"beta_adjusted non-zero: {(beta_adjusted > 1e-15).sum().item()}")
...
def run_vae(total_mut_counts, valid_alleles, mut_counts,
    ...
    lb_factor = 0.90 # uninformed, lowerbound and upperbound of the coverage wrt 1x expected coverage.
    ub_factor = 1.10
    ...
    sparse_prior_mu, sparse_prior_logvar = create_sparse_priors(sample, db)
