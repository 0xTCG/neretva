...
if __name__ == "__main__":
    ...
    allele_cns, obj, recon_err, base_cost = run_ilp(
        total_mut_counts,   
        valid_allele_names,   
        mut_counts_tensor,       
        sample=sample,
        db=db,
        max_cn=4,
        base_cost_weight=0.01,
        time_limit=1800
    )
    
    #%%
    # Group by major allele type (same grouping as VAE version)
    major_allele_cns = {}
    for allele_name, cn in allele_cns.items():
        ...
        if major_key not in major_allele_cns:
            major_allele_cns[major_key] = 0
        major_allele_cns[major_key] += cn

    print("\n[Alleles]")
    for major_key, cn in sorted(major_allele_cns.items()):
        for _ in range(cn):
            print(f"{major_key}")
# %%