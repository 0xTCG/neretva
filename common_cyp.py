...
class Position:
    def __init__(self, gene, position, ref_base=None):
        ...
        if not ref_base:
            ...
            self.ref_base = gene.aldy_gene[position]
    ...
    def populate_positions(self):
        ...
        for gene in self.genes.values():
            ...
            for pos in all_pos:
                ...
                for variant in ['A', 'C', 'G', 'T', 'N', 'P', 'M']:
                    position.variants[variant] = Variant(position, variant)
                
                # Initialize multi_snp_ops
                position.multi_snp_ops = set()
        ...
        for gene_name in sorted(self.genes.keys()):
            ...
            for allele in gene.alleles.values():
                ...
                for position in gene.positions.values():
                    ...
                    has_multi_snp = any((pos, op) in allele.ops 
                                    for pos, op in allele.ops 
                                    if pos == position.position and '.' in op and '>' in op)
                    
                    has_snp = any((pos, op) in allele.ops 
                                for pos, op in allele.ops 
                                if pos == position.position and '>' in op and '.' not in op)
